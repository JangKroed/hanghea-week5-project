...
describe('post service Layer Test', () => {
  ...
  test('findPostById fail case', async () => {
    postService.postRepository.findPostById = jest.fn(() => {
      return null;
    });
    try{
  ...
  test('createPost sucess case ', async () => {
    ...
    const createPostData = await postService.createPost(
      userId,
      nickname,
      title,
      content
  ...
  test('updatePost sucesscase', async () => {
    ...
    const updatePostData = await postService.updatePost(
      postId,
      userId,
      title,
      content
  ...
  test('updatePost fail case', async () => {
    postService.postRepository.updatePost = jest.fn(() => {
      return updatePostOutput;
    });
    ...
    // updatePost는 몇번 호출되는가
    expect(postService.postRepository.updatePost).toHaveBeenCalledTimes(0);
  ...
  test('deletePost fail case', async () => {
    postService.postRepository.deletePost = jest.fn(() => {
      return updatePostOutput;
    });
    ...
    // deletePost는 몇번 호출되는가
    expect(postService.postRepository.deletePost).toHaveBeenCalledTimes(0);
